...
import test.yuedong.com.myapplication.refreshlayout.inner.OnGestureListener;
import test.yuedong.com.myapplication.refreshlayout.inner.PullListener;
import test.yuedong.com.myapplication.refreshlayout.inner.RefreshListenerAdapter;
import test.yuedong.com.myapplication.refreshlayout.processor.AnimProcessor;
import test.yuedong.com.myapplication.refreshlayout.processor.IDecorator;
import test.yuedong.com.myapplication.refreshlayout.processor.OverScrollDecorator;
import test.yuedong.com.myapplication.refreshlayout.processor.RefreshProcessor;
...
public class TwinklingRefreshLayout extends RelativeLayout implements PullListener, NestedScrollingChild {
    ...
    public static final int HEADER_VIEW_TYPE_NORMAL = 0;
    public static final int HEADER_VIEW_TYPE_MAIN = 1;
    //底部样式，根据需要可以自己更改
    private int footerViewType = 0;
    public static final int FOOTER_VIEW_TYPE_NORMAL = 0;
    public static final int FOOTER_VIEW_TYPE_MAIN = 1;
    ...
    public TwinklingRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            headerViewType = a.getInt(R.styleable.TwinklingRefreshLayout_tr_header_type, 0);
            footerViewType = a.getInt(R.styleable.TwinklingRefreshLayout_tr_footer_type, 0);
        } finally {
            a.recycle();
        }
    ...
    private void addHeader() {
        ...
        //直接添加我们自己的头 写死，不另外配置
        setHeaderView(chooseHeaderView());
    }


    private IHeaderView chooseHeaderView() {
        switch (headerViewType) {
            case HEADER_VIEW_TYPE_NORMAL:
                return new HeadView(getContext());
            case HEADER_VIEW_TYPE_MAIN:
                return new MainHeadView(getContext());
            default:
                return new HeadView(getContext());
        }
    }
    ...
    private IBottomView chooseFootView() {
        switch (footerViewType) {
            case FOOTER_VIEW_TYPE_NORMAL:
                return new FootView(getContext());
            case FOOTER_VIEW_TYPE_MAIN:
                return new MainFootView(getContext());
            default:
                return new FootView(getContext());
        }
    }
